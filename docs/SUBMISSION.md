...
| 4 | Production scaffolding | `src/obs/` (pino + JSONL tracer), `src/resilience/` (retry/backoff, token-bucket rate limit, typed errors), `src/eval/` (harness), `tests/` (51 tests), Dockerfile, `.github/workflows/ci.yml`. |
...
npm run eval                # 4/4 deterministic eval tasks (3 fixtures), no API key
